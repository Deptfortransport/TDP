...
::
::  Batch File:      	IF070 (far265) import batch file
::  Author:          	Steve Craddock    
::
::  Built/Tested On: 	Windows 2008 Server
::
::  Purpose:  	     	general framework batch file.
...
::
:: Note zip file contains payload file called IF070_037_yyyymmddhhmm.csv this must be 
:: renamed to be airOperators.csv
::


for /L %%a in (1,1,%items%) do ( echo %date% %time% %procname% going to copy .csv file from D:\Gateway\dat\processing\%procname%\ to !%str1%%%a!\airOperators.csv >> %OUTPUT_FILE%
				echo %items% 
				echo %%a
				xcopy /y D:\Gateway\dat\processing\%procname%\*.csv !%str1%%%a!\airOperators.csv*
