...
(function($) { 
	jQuery.fn.smoothDivScroll = function(options){
		...
		var defaults = {
		scrollingHotSpotLeft: "div.scroll-left", // The hot spot that triggers scrolling left.
		scrollingHotSpotRight: "div.scroll-right", // The hot spot that triggers scrolling right.
		...
		return this.each(function() {
			...
			$(window).one("load",function(){
				...
				if(options.ajaxContentURL.length === 0) {
					...
					// CHANGE BY ANDYT - add 50 to the width to give some padding on the end
					$mom.scrollableAreaWidth = $mom.scrollableAreaWidth + 50;
			...
			$mom.find(options.scrollingHotSpotRight).bind('mousemove',function(e){
				...
				scrollXpos = Math.round((hotSpotWidth/hotSpotWidth) * options.scrollingSpeed);
