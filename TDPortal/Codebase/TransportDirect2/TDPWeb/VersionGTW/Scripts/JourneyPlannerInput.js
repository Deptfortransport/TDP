...
function setupDropDowns() {
    $('div.sjpContent div.locationControl select.locationDrop').selectmenu({ width: 270, maxHeight: 150, positionOptions: { collision: "flip"} });
    $('div.sjpContent div.eventDate select').selectmenu({ maxHeight: 120, positionOptions: { collision: "flip"} });
    $('div.selectJourneyDrop select').selectmenu({ width: 245, maxHeight: 120, positionOptions: { collision: "flip"} });
...
function initCalendar() {
    ...
    $("input.dateEntry").dateEntry({
        ...
        dateFormat: 'dd/mm/yy',
        showOn: "button",
        buttonImage: "./../versiongtw/images/icons/calendaricon.png",
...
function setupMobilityNeeds() {
    ...
    if ($('div.pjOptions :radio[checked]').length > 0 && !($('div.pjOptions :radio:last').attr('checked'))) {
        ...
        $('div.pjOptionsHead').find('input.js_accessiblity_btn').addClass('expanded');
    }
    ...
    $('div.pjOptionsHead').bind('click', function (event) {
        if ($(this).next().hasClass('collapsed')) {
            ...
            $(this).find('input.js_accessiblity_btn').addClass('expanded');
        }
        else {
            $(this).next().hide('blind', 300).switchClass('expanded', 'collapsed');
            $(this).find('input.js_accessiblity_btn').removeClass('expanded');
        }
...
function setupInformationTips() {
    ...
    // Date and time
    $('div.eventDate a.tooltip_information').qtip({
        position: {
            my: 'bottom center',
            at: 'top center',
            container: $('.sjpContent'),
