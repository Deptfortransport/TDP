...
namespace TDP.UserPortal.JourneyControl
{
    ...
    public class JourneyNoteFilter
    {
        ...
        public static JourneyNoteFilter Current
        {
            get
            {
                return (JourneyNoteFilter)TDPServiceDiscovery.Current.Get<JourneyNoteFilter>(ServiceDiscoveryKey.JourneyNoteFilter);
            }
        }
        ...
        /// </summary>
        public bool DisplayNote(TDPModeType modeType, string regionId, bool accessibleJourney, string journeyNote)
        {
        ...
        private void Load()
        {
                ...
                using (SqlHelper helper = new SqlHelper())
                {
                    ...
                    if (TDPTraceSwitch.TraceVerbose)
                    {
                        Logger.Write(new OperationalEvent(TDPEventCategory.Business,
                            TDPTraceLevel.Verbose, "Loading JourneyNotesFilter data started"));
                    }
                    ...
                    {
                        Logger.Write(new OperationalEvent(TDPEventCategory.Business,
                            TDPTraceLevel.Verbose, "Loading JourneyNotesFilter data completed"));
                    }
                    #endregion
                }
            }
            ...
            {
                Logger.Write(new OperationalEvent(TDPEventCategory.Database, TDPTraceLevel.Error, "An SQL exception occurred whilst attempting to load the JourneyNotesFilter data.", sqle));
            }
            catch (Exception ex)
            {
                Logger.Write(new OperationalEvent(TDPEventCategory.Database, TDPTraceLevel.Error, "An exception occurred whilst attempting to load the JourneyNotesFilter data.", ex));
            }
        }
