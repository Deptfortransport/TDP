<%@ Control Language="c#" AutoEventWireup="True" Codebehind="JourneyAdjustTableGridControl.ascx.cs" Inherits="TransportDirect.UserPortal.Web.Controls.JourneyAdjustTableGridControl" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<asp:repeater id="detailsRepeater" runat="server">
	<headertemplate>
		<table class="jdtgrid" cellspacing="0" cellpadding="5px" summary="<%= GetMainTableSummary %>">
			<thead>
				<tr>
					<%# HeaderItem(0) %>
					<%# HeaderItem(1) %>
	...
	<itemtemplate>
		<tr id="<%# GetHighlightRowId( Container.ItemIndex) %>" class="<%# GetHighlightRowClass( Container.ItemIndex) %>">
			<%# CellStart(0) %>
			&nbsp;
			<cc1:tdimage id="selectedTick" runat="server" style="<%# GetVisibility(Container.ItemIndex) %>" enableviewstate="false"></cc1:tdimage>
			&nbsp;
			<%# CellEnd(0) %>
