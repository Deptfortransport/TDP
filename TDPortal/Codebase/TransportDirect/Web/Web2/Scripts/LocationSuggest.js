...
(function($) {
    ...
    $.fn.locationSuggest = function(options) {
        ...
        return this.each(function(x) {
            ...
            var results_container = $('<ul id="' + x_id + '" class="locationSuggestList" role="menu" ></ul>').css("position", "absolute").hide();

            // Position is set when showing the list
            ...
            $('.bodyArea').append(results_container);
            //results_container.before(sclabel);
            $('.bodyArea').append(sclabel);
            ...
            input.one("removeLocationSuggest", function() {
                // Removes attached events in the namespace
                $(this).unbind('.locationSuggest');
            });
            // END EVENTS
            // ------------------
            ...
            function setupList() {
                ...
                // Re-position incase the input box has moved
...
//                    }
    ...
    // Remove location suggest plugin
    $.fn.removeLocationSuggest = function() {

        // Fire the event to remove
        return this.trigger("removeLocationSuggest");
    }
...
$(document).ready(function () {
    ...
    setupLocationInputMore();
    setupLocationInputSuggest();
...
// Setsup the location suggest functionality for location input controls
function setupLocationInputSuggest() {

    $('.locationBox').each(function () {

        // Only setup location suggest for the location controls which require it
...
// Removes the location suggest functionality when the location input more button is selected
function setupLocationInputMore() {

    $('.locationControl .moreOptions').click(function(event) {

        // Find location input in the parent container
        var $locationinput = $(this).closest('.locationControl').find('.locationBox');

        // Remove auto suggest functionality
        $locationinput.removeLocationSuggest();

        // Clear the hidden input fields when user types again
        $locationinput.one('keydown', function() {
            $('#' + $(this).attr('id') + '_hdnValue').attr('value', '');
            $('#' + $(this).attr('id') + '_hdnType').attr('value', '');
        });
    });
}