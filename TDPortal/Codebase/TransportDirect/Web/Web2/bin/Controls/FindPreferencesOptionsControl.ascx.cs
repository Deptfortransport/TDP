// *********************************************** 
// NAME                 : FindPreferencesOptionsControl.ascx.cs
// AUTHOR               : Jonathan George
// DATE CREATED         : 12/07/2004
// DESCRIPTION  : Control to display preferences header bar
// ************************************************ 
// $Log:   P:/TDPortal/archives/DotNet2.0Codebase/TransportDirect/Web2/Controls/FindPreferencesOptionsControl.ascx.cs-arc  $ 
//
...
namespace TransportDirect.UserPortal.Web.Controls
{
	...
	public partial  class FindPreferencesOptionsControl : TDUserControl
	{
		#region Controls
				
		// Keys used to obtain strings from the resource file
		private const string HidePreferencesImageUrlKey = "FindPreferencesOptionsControl.HidePreferences.Url.{0}";
		private const string HidePreferencesImageAltTextKey = "FindPreferencesOptionsControl.HidePreferences.Alt.{0}";
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			preferencesHeaderLabel.Text = rm.GetString(String.Format(PreferencesHeaderLabelTextKey, this.PageId.ToString()), TDCultureInfo.CurrentUICulture);

			if (PageId == PageId.FindCarInput || PageId == PageId.ParkAndRideInput)
			{
				preferencesHeaderLabel.Text = rm.GetString(String.Format(PreferencesCarHeaderLabelTextKey, this.PageId.ToString()), TDCultureInfo.CurrentUICulture);
			}
			...
			if( !IsPostBack )
			{
				...
				// Only display the submit button for the following pages
                // CCN 0427 Removed the VisitPlannerInput page to show submit button
		...
		/// </summary>
		public bool AllowSubmitButton
		{
			get { return commandSubmit.Visible; }
			set { commandSubmit.Visible = value; }
		}
		...
		public Label LabelPreferencesHeader
		{
			get {return preferencesHeaderLabel;}
			set {preferencesHeaderLabel = value;}
		}
		...
		private void OnCommandButtonClick(object sender, EventArgs e)
		{
			...
			if (sender.Equals(commandSubmit))
