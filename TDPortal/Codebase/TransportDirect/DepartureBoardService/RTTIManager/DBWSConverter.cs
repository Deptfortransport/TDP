...
namespace TransportDirect.UserPortal.DepartureBoardService.RTTIManager
{
    ...
    internal class DBWSConverter
    {
        ...
        private LookupTranslation lookUpTranslation;
        ...
        public DBWS.DBWSRequest BuildDBWSRequest(
            string requestId,
            DBSLocation stopLocation, DBSLocation filterLocation, 
            ...
            TemplateRequestType requestType)
        {
            ...
            try
            {
                if (requestType == TemplateRequestType.TripRequestByCRS)
                {
                    // Build station board 
                    if (!ExtractDataForStationBoard(dbwsResult, result, stopLocation, requestedDate))
                    {
                        if (IsInvalidRequestError(dbwsResult))
                        {
                            ...
                            RTTIUtilities.BuildRTTIError(Messages.RTTIRequestInValid, result);
                            result.Messages[0].Description += string.Format(" {0}", dbwsResult.Messages[0].Description);
                        }
                        else
                            RTTIUtilities.BuildRTTIError(Messages.RTTIUnableToExtract, result);
                    }
                }
                else if (requestType == TemplateRequestType.TrainRequestByRID)
                {
                    ...
                    throw new Exception(string.Format("DBWSConverter BuildDBSResult called with unsupported request type{{0}]", requestType));
            }
            catch (Exception ex)
            {
                // Build error message 
        ...
        private bool ExtractDataForStationBoard(DBWS.DBWSResult dbwsResult, DBSResult result,
            DBSLocation stopLocation, DateTime stopDate)
        {
            try
            {
                ...
                if (dbwsResult.Location != null && !string.IsNullOrEmpty(dbwsResult.Location.LocationCRS))
                {
                    ...
                    requestedStationNaptan = stopLocation.NaptanIds[0]; // Assume there is a naptan
        ...
        private string GetStationNameForNaptan(string naptanId)
        {
            if (naptanId == null || naptanId.Length == 0)
                ...
                return StopEventManager.StopEventConverter.DecorateStopName(
                    additionalData.LookupStationNameForNaptan(naptanId).ToString());
        }
        ...
        private DateTime ParseTime(string time, DateTime date)
        {
            ...
            try
            {
                ...
                if (hour < int.Parse(Properties.Current[Keys.RTTIFirstServiceHour]))
                {
        ...
        private bool IsInvalidRequestError(DBWS.DBWSResult dbwsResult)
        {
            ...
            {
                if (dbwsResult.Messages[0].Code == (int)TDExceptionIdentifier.DBWSLDBRequestInvalid)
                    return true;
            }
