...
/*
// This class has neeb downloaded from microsoft site. 
...
namespace TransportDirect.Common.DatabaseInfrastructure {
    ...
    public class XmlCsvReader : XmlReader {
        ...
        public XmlCsvReader(Uri location, XmlNameTable nametable) {
            _baseUri = location;
            _nt = nametable;
            if (nametable == null) _nt = new NameTable();
            _csvReader = new CsvReader(location, null, 4096);
        }
        ...
        /// <param name="nametable">The nametable to use for atomizing element names</param>
        public XmlCsvReader(Stream input, Uri baseUri, XmlNameTable nametable) {
            _baseUri = baseUri;
    ...
    class CsvReader {
        ...
        public CsvReader(Uri location, string proxy, int bufsize) {  // the location of the .csv file
            if (location.IsFile) {
